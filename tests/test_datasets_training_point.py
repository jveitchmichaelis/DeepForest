...
def test_gaussian_density_count_normalization(point_csv, point_root_dir):
    ...
    points = torch.tensor([[50.0, 50.0], [60.0, 60.0], [70.0, 70.0]], dtype=torch.float32)
    labels = torch.tensor([0, 0, 1], dtype=torch.int64)  # 2 Trees, 1 Shrub
    density = ds.gaussian_density(points, labels, (3, 200, 200))

    assert density[0].sum().item() == pytest.approx(2.0, abs=1e-3)
    assert density[1].sum().item() == pytest.approx(1.0, abs=1e-3)
