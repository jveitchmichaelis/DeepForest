...
@pytest.mark.parametrize("architecture", ["retinanet", "DeformableDetr"])
def test_checkpoint_label_dict(config, tmpdir):
    ...
    config["train"]["csv_file"] = os.path.join(tmpdir, "example.csv")
    config["train"]["root_dir"] = os.path.dirname(csv_file)
    config["train"]["fast_dev_run"] = True
    config["validation"]["csv_file"] = os.path.join(tmpdir, "example.csv")
...
def test_load_from_checkpoint_with_overrides(config):
    ...
    loaded = main.deepforest.load_from_checkpoint(
        checkpoint_path,
        weights_only=True,
        config_args={"batch_size": new_batch_size, "train": {"epochs": new_epochs}},
    )
...
# This test requires "good' weights for tree detection
@pytest.mark.parametrize("architecture", ["retinanet"])
def test_save_and_reload_weights(config, tmpdir):
    """Test saving and loading raw PyTorch state dict (not Lightning checkpoint)"""
    img_path = get_data(path="2019_YELL_2_528000_4978000_image_crop2.png")
    ...
    # Save raw state dict
    weights_path = f"{tmpdir}/weights.pt"
...
@pytest.mark.parametrize("architecture", ["retinanet", "DeformableDetr"])
def test_reload_multi_class(config, tmpdir):
